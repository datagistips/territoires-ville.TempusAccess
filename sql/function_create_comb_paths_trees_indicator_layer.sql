...
BEGIN
    ...
    LOOP
        IF (r.col_name = 'total_time') -- Total time to the destination node of the section
        THEN
            indics_str = indics_str || $$round(CASE WHEN $$ || day_ag || $$ = 1 THEN avg(total_time) WHEN $$ || day_ag || $$ = 2 THEN min(total_time) WHEN $$ || day_ag || $$ = 3 THEN max(total_time) END::numeric, 2) AS total_time, $$;
        END IF;
