...
BEGIN
    ...
    LOOP
        ...
        THEN 
            indics_str = indics_str || $$st_length(st_transform(geom, 2154))/1000 as total_dist, $$;
        END IF; 
        ...
        THEN 
            indics_str = indics_str || $$(st_length(st_transform(geom, 2154))/1000)/(extract(hour from (d_time - o_time)) + extract(minute from (d_time - o_time))/60.0 + extract(second from (d_time - o_time))/3600.0) as speed_kmh, $$;
        END IF;
    END LOOP; 
