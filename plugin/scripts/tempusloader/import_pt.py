...
def import_pt_gtfs(args):
    ...
    gtfsi = provider.ImportPTGTFS(path=args.path, prefix="", dbstring=args.dbstring, logfile=args.logfile, options={'g':'geom', 'D':True, 'I':True, 'S':True}, sep=",", \
                                  encoding=args.encoding, copymode=args.copymode, doclean=not args.noclean, subs=subs)
    return gtfsi.load()
