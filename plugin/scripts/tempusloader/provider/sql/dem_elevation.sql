...
(
select
	id, st_setsrid(st_makepoint(st_x(pt.geom), st_y(pt.geom), st_value( rast, pt.geom )), 4326) as geomz
from
	dem
join
...
select
	id,
	st_makeline( 
	st_setsrid(st_makepoint(st_x(geom), st_y(geom), st_value(rast, geom)), 4326)
	) as geomz
from
	pts
join
