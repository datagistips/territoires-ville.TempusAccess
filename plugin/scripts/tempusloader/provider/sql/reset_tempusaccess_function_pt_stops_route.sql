CREATE OR REPLACE FUNCTION tempus_access.create_pt_stops_route_indicator_layer(
...
BEGIN
        ...
        (
            -- Apply filters
                SELECT feed_id, 
                       ...
                       array_agg(stop_id_int ORDER BY stop_sequence) as stop_ids_int, 
                       min(departure_time ORDER BY stop_sequence) as departure_time,
                       max(arrival_time ORDER BY stop_sequence) as arrival_time, 
                       route_id_int, 
                       route_id, 
                       route_short_name, 
                       route_long_name, 
    ...
    LOOP 
        ...
        ELSE
            indics_time_ag = indics_time_ag || (SELECT coalesce(replace(time_ag_stops_routes, '%(time_ag)', time_ag_str::character varying) || ' AS ' || r.col_name || ', ', '') FROM tempus_access.indicators WHERE col_name = r.col_name);
            indics_day_ag = indics_day_ag || (SELECT coalesce(replace(day_ag_stops_routes, '%(day_ag)', day_ag_str::character varying) || ' AS ' || r.col_name || ', ', '') FROM tempus_access.indicators WHERE col_name = r.col_name);         
        END IF;
    END LOOP; 
    ...
    s= $$DROP TABLE IF EXISTS daily_aggregated_data; 
        CREATE TEMPORARY TABLE daily_aggregated_data AS
         (
                ...
                (
                    SELECT row_number() over() as stops_route_id_int, *
                    FROM (
                            SELECT stop_ids_int, route_id_int, stop_names, route_type
                            FROM filtered_data                        
                            GROUP BY stop_ids_int, 
                                     route_id_int, 
                                     stop_names, 
                                     route_type
                            ORDER BY route_id_int, stop_names
                         ) q
                ),  
                all_days AS
                (
                    SELECT DISTINCT stops_routes.*, q.day_date
                    FROM stops_routes
                    CROSS JOIN (SELECT unnest('$$ || days_list || $$'::date[]) AS day_date) q
                )
                ( -- Mode by mode
                    SELECT 
                               all_days.stops_route_id_int, 
                               feed_id, 
                               array_agg(trip_id_int) as trip_ids_int, 
                               all_days.stop_names,
                               all_days.stop_ids_int, 
                               ...
                               agency_id,
                               agency_name, 
                               min(geom_trip) as geom_trip, 
                               all_days.day_date, 
                               $$ || indics_time_ag || $$
                               FROM filtered_data 
                               RIGHT JOIN all_days 
                               ON (all_days.stop_ids_int = filtered_data.stop_ids_int) AND (all_days.route_id_int = filtered_data.route_id_int) AND (all_days.day_date = filtered_data.date) AND (all_days.route_type = filtered_data.route_type)
                    GROUP BY all_days.stops_route_id_int, 
                             feed_id, 
                             all_days.stop_names,
                             all_days.stop_ids_int, 
                             all_days.route_id_int, 
                ...
                -- Considering all modes together has no signification for stops routes indicators 
                -- since they are designed to be served by only one mode. 
         )$$;
    RAISE NOTICE '%', s;
    EXECUTE(s);
    ...
    s = $$
        DROP TABLE IF EXISTS indic.stops_routes;
        CREATE TABLE indic.stops_routes AS
        (
            SELECT row_number() over() AS gid, *
            FROM
                (
                    SELECT stops_route_id_int, 
                           min(trip_ids_int) AS trip_ids_int, 
                           min(feed_id) AS feed_id,
                           min(stop_names) AS stop_names,
                           min(stop_ids_int) AS stop_ids_int,
                           ...
                           array_remove(array_agg(CASE WHEN trip_ids_int IS NOT NULL THEN day_date END ORDER BY day_date), NULL) AS services_days,
                           array_agg(day_date ORDER BY day_date) AS days, 
                           $$ || indics_day_ag || $$, 
                           min(geom_trip) AS geom
                    FROM daily_aggregated_data
                    GROUP BY stops_route_id_int, 
                             route_type
                    ORDER BY stops_route_id_int, 
                             route_type
                ) q
        );
        $$;
    ...
    THEN 
        ...
        s = $$
                ALTER TABLE indic.stops_routes
                ADD COLUMN zones_list character varying[];
                
                WITH stops_zones AS (
                    SELECT stops_routes.gid, array_agg(DISTINCT zoning.vendor_id) as zones_list
                    FROM indic.stops_routes JOIN tempus_gtfs.stops ON (ARRAY[stops.id] <@ stops_routes.stop_ids_int)
                                            JOIN zoning.$$ || zoning_table || $$ zoning ON (st_within(stops.geom, zoning.geom))
                    GROUP BY stops_routes.gid
                )
                UPDATE indic.stops_routes
                SET zones_list = stops_zones.zones_list
                FROM stops_zones
                WHERE stops_zones.gid = stops_routes.gid;
            $$;
        RAISE NOTICE '%', s;
        EXECUTE(s);
    END IF; 
    ...
    s=$$
    ALTER TABLE indic.stops_routes ADD COLUMN symbol_size real;
    ALTER TABLE indic.stops_routes ADD COLUMN symbol_color real;
    ALTER TABLE indic.stops_routes ADD CONSTRAINT stops_routes_pkey PRIMARY KEY(gid);
    $$;
    EXECUTE(s);
       
    t=$$DELETE FROM tempus_access.indic_catalog WHERE layer_name = 'stops_routes';
    ...
    VALUES ('stops_routes', 
           (SELECT code FROM tempus_access.obj_type WHERE def_name = 'stops_routes')::character varying, 
