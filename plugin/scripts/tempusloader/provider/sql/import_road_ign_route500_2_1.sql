...
SELECT pgr_createTopology('%(temp_schema).troncon_route',  0.01, 'geom_2d', 'id_rte500');
...
(
    SELECT (select id from %(temp_schema).troncon_route_vertices_pgr_idmap WHERE vendor_id = troncon_route_vertices_pgr.id::character varying),
           null, 
...
INSERT INTO %(temp_schema).troncon_route_idmap (id_rte500)
    SELECT id_rte500 FROM %(temp_schema).troncon_route;
       
CREATE INDEX troncon_route_idmap_id_rte500_idx on %(temp_schema).troncon_route_idmap(id_rte500); 
...
SELECT
	(select id from %(temp_schema).troncon_route_idmap where id_rte500=troncon_route.id_rte500) as id, 
	...
	CASE vocation
		...
		ELSE 5 -- Others
	END AS road_type,	
	(select id from %(temp_schema).troncon_route_vertices_pgr_idmap where vendor_id = source::character varying), 
    (select id from %(temp_schema).troncon_route_vertices_pgr_idmap where vendor_id = target::character varying), 
	CASE
...
insert into %(temp_schema).communication_restreinte_idmap (id_rte500)
       select id_rte500 from %(temp_schema).communication_restreinte;
       
create index communication_restreinte_idmap_id_rte500_idx on %(temp_schema).communication_restreinte_idmap(id_rte500);

INSERT INTO tempus.road_restriction(id, network_id, vendor_id, sections)
SELECT (select id from %(temp_schema).communication_restreinte_idmap where id_rte500=communication_restreinte.id_rte500) as id, 
       (select max(id) from tempus.road_network where name = '%(source_name)'),
       id_rte500, 
       array[(select id from %(temp_schema).troncon_route_idmap where id_rte500 = id_tro_ini), (select id from %(temp_schema).troncon_route_idmap where id_rte500= id_tro_fin)]
...
SELECT
        (select id from %(temp_schema).communication_restreinte_idmap where id_rte500=communication_restreinte.id_rte500) as id,
        0 as period_id,
