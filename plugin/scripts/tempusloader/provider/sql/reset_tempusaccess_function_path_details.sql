...
BEGIN
        ...
        (
            SELECT path_id, 
                   ...
                   CASE WHEN tempus_paths_results.step_type = 1 THEN (SELECT st_multi(st_force2d(tempus_access.pt_section(stop_o.id, stop_d.id, tempus_paths_results.pt_trip_id::integer)))) 
                        WHEN ARRAY[tempus_paths_results.step_type] <@ ARRAY[0, 2] AND (source_road_vertex_id IS NOT NULL AND target_road_vertex_id IS NOT NULL) THEN (SELECT st_multi(st_force2d(geom)) FROM tempus.road_section WHERE (node_from = tempus_paths_results.target_road_vertex_id AND node_to = tempus_paths_results.source_road_vertex_id) OR (node_from = tempus_paths_results.source_road_vertex_id AND node_to = tempus_paths_results.target_road_vertex_id))
                        WHEN ARRAY[tempus_paths_results.step_type] <@ ARRAY[0, 2] AND (road_edge_id IS NOT NULL) THEN (SELECT st_multi(st_force2d(geom)) FROM tempus.road_section WHERE id = road_edge_id)
                        WHEN ARRAY[tempus_paths_results.step_type] <@ ARRAY[0, 2] AND (source_road_vertex_id IS NOT NULL AND target_pt_stop_id IS NOT NULL) THEN (SELECT st_multi(st_force2d(tempus_access.road_section(target_pt_stop_id::integer, source_road_vertex_id::integer))))
                        WHEN ARRAY[tempus_paths_results.step_type] <@ ARRAY[0, 2] AND (target_road_vertex_id IS NOT NULL AND source_pt_stop_id IS NOT NULL) THEN (SELECT st_multi(st_force2d(tempus_access.road_section(source_pt_stop_id::integer, target_road_vertex_id::integer))))
                   END AS geom
    ...
    LOOP 
        indics_str = indics_str || (SELECT coalesce(replace(day_ag_paths_details, '%(day_ag)', indics_str::character varying) || ' AS ' || r.col_name || ', ', '') FROM tempus_access.indicators WHERE col_name = r.col_name);         
    END LOOP; 
