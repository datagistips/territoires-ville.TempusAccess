-- Tempus - PT merge Wrapper

CREATE OR REPLACE FUNCTION tempus_gtfs.export_pt(feed_id character varying) 
RETURNS boolean AS
$$
    DROP SCHEMA IF EXISTS _tempus_export CASCADE;
    CREATE SCHEMA _tempus_export; 

    CREATE TABLE _tempus_export.agency AS
    (
        SELECT agency_id,agency_name,agency_url,agency_timezone,agency_lang 
        FROM tempus_gtfs.agency 
        WHERE feed_id = $1
    );
    CREATE TABLE _tempus_export.calendar AS
    (
    ...
    CREATE TABLE _tempus_export.stop_times AS
    (
    ...
    CREATE TABLE _tempus_export.stops AS
    (
    ...
    (
        SELECT shape_id, shape_pt_lat, shape_pt_lon, row_number() over(partition by shape_id) as shape_pt_sequence 
            FROM (
                ...
                WHERE feed_id = IN (SELECT feed_id FROM tempus_gtfs.feed_info WHERE ARRAY[id] <@ $1)
                ORDER BY shape_id, (st_dumppoints(geom)).path
            ) q
    );
    CREATE TABLE _tempus_export.fare_attributes AS
    (
        SELECT fare_id, price, currency_type, payment_method, transfers, agency_id, transfer_duration 
            FROM tempus_gtfs.fare_attributes
            WHERE feed_id = $1
    ); 
    CREATE TABLE _tempus_export.fare_rules AS
    (
