-- Tempus - Road Route120 SQL import Wrapper
 /*
...
SELECT pgr_createTopology('%(temp_schema).troncon_route',  0.01, 'geom_2d', 'id_rte120');
...
INSERT INTO %(temp_schema).troncon_route_idmap (id_rte120)
    SELECT id_rte120 FROM %(temp_schema).troncon_route;
       
CREATE INDEX troncon_route_idmap_id_rte120_idx on %(temp_schema).troncon_route_idmap(id_rte120); 
...
SELECT
	(select id from %(temp_schema).troncon_route_idmap where id_rte120=troncon_route.id_rte120) as id, 
	id_rte120::character varying as vendor_id, 
    (select max(id) from tempus.road_network where name = '%(source_name)') as network_id, 
	CASE vocation
...
insert into %(temp_schema).communication_restreinte_idmap (id_rte120)
       select id_rte120 from %(temp_schema).communication_restreinte;
       
create index communication_restreinte_idmap_id_rte120_idx on %(temp_schema).communication_restreinte_idmap(id_rte120);


INSERT INTO tempus.road_restriction(id, network_id, vendor_id, sections)
SELECT (select id from %(temp_schema).communication_restreinte_idmap where id_rte120=communication_restreinte.id_rte120) as id, 
       (select max(id) from tempus.road_network where name = '%(source_name)'),
       id_rte120, 
       array[(select id from %(temp_schema).troncon_route_idmap where id_rte120 = id_tro_ini), (select id from %(temp_schema).troncon_route_idmap where id_rte120= id_tro_fin)]
...
SELECT
        (select id from %(temp_schema).communication_restreinte_idmap where id_rte120=communication_restreinte.id_rte120) as id,
        0 as period_id,
