	...
	s=$$
        ...
        (
            SELECT 
               ...
               $$ || indics_str || $$
               CASE WHEN road_section_id IS NOT NULL THEN road_section.geom END AS geom_section, 
            ...
            FROM tempus_access.tempus_paths_tree_results JOIN tempus.transport_mode ON (tempus_paths_tree_results.transport_mode = transport_mode.id)
                                                         JOIN tempus.road_section ON (tempus_paths_tree_results.road_section_id = road_section.id) 
        );
