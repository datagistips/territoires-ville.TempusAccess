...
CREATE TABLE %(temp_schema).temp AS
(
...
(
    ...
    FROM tempus_gtfs.calendar_dates JOIN %(temp_schema).temp ON calendar_dates.feed_id || '-' || calendar_dates.service_id = split_part(stop_times.service_id, ';', 1)
...
(
    ...
    FROM %(temp_schema).temp JOIN tempus_gtfs.trips ON (ARRAY[feed_id] <@ ARRAY['ter', 'ic'] AND trips.trip_id = temp.trip_id)
    WHERE feed_id IN (SELECT feed_id FROM tempus_gtfs.feed_info WHERE ARRAY[id] <@ ARRAY['ter', 'ic'])
);
