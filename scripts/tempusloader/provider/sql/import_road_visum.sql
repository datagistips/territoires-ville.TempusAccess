-- Tempus - Visum SQL import Wrapper
 /*
        ...
        %(temp_schema): name of the temporary schema where data are loaded
...
SELECT DISTINCT
	(SELECT id FROM %(temp_schema).road_node_idmap WHERE vendor_id = %(node_NO)::character varying) as id,
...
ANALYSE %(temp_schema).node;
ANALYSE %(temp_schema).link;
CREATE INDEX idx%(temp_schema)_node_id ON %(temp_schema).node(%(node_NO));
CREATE INDEX idx%(temp_schema)_link_id ON %(temp_schema).link(%(link_NO));
...
DROP TABLE IF EXISTS %(temp_schema).road_link_idmap;
CREATE TABLE %(temp_schema).road_link_idmap
(
        id bigserial primary key,
        vendor_id varchar
);

SELECT setval('%(temp_schema).road_link_idmap_id_seq', (SELECT CASE WHEN max(id) IS NULL THEN 1 ELSE max(id)+1 END FROM tempus.road_section), False);
INSERT INTO %(temp_schema).road_link_idmap (vendor_id)
       SELECT %(link_NO) 
       FROM %(temp_schema).link
       ORDER BY %(link_NO);
CREATE INDEX road_link_idmap_vendor_id_idx ON %(temp_schema).road_link_idmap(vendor_id); 


-- Create temporary table for database populating (tables road_section, road_daily_profile, and road_section_speed)
CREATE TABLE %(temp_schema).full_info AS
(
    SELECT
...
SELECT DISTINCT ON (section_id)
	(SELECT id FROM %(temp_schema).road_link_idmap WHERE vendor_id = section_id::character varying) as id,
    (SELECT max(id) FROM tempus.road_network WHERE name = '%(source_name)') as network_id, 
    section_id::character varying AS vendor_id,
	(SELECT id FROM %(temp_schema).road_node_idmap WHERE vendor_id = node_from::character varying) AS node_from,
	(SELECT id FROM %(temp_schema).road_node_idmap WHERE vendor_id = node_to::character varying) AS node_to,
	...
	tollway AS tollway,
	geom AS geom
FROM %(temp_schema).full_info
WHERE traffic_rules_ft > 0 OR traffic_rules_tf > 0;

CREATE TABLE %(temp_schema).profile
(
    id serial,
...
SELECT setval('%(temp_schema).profile_id_seq', (SELECT CASE WHEN max(profile_id) IS NULL THEN 1 ELSE max(profile_id)+1 END FROM tempus.road_daily_profile), False);

INSERT INTO %(temp_schema).profile(free_flow_speed, loaded_speed, road_section_ids)
SELECT free_flow_speed, loaded_speed, array_agg((SELECT id FROM %(temp_schema).road_link_idmap WHERE vendor_id = section_id::character varying)) as road_section_ids 
FROM %(temp_schema).full_info 
...
CREATE TABLE %(temp_schema).restriction
(
    id serial, 
    sections integer[],
...
SELECT setval('%(temp_schema).restriction_id_seq', (SELECT CASE WHEN max(id) IS NULL THEN 1 ELSE max(id)+1 END FROM tempus.road_restriction), False);

INSERT INTO %(temp_schema).restriction(sections, time_penalty)
(
	SELECT ARRAY[road_section1.id, road_section2.id] as sections, 
           mov.%(mov_T0TI) as time_penalty
	FROM %(temp_schema).mov JOIN tempus.road_section as road_section1 
                                ON (road_section1.node_from = (SELECT id FROM %(temp_schema).road_node_idmap WHERE vendor_id = mov.%(mov_NUMNOEUDO)::character varying) 
                                   AND road_section1.node_to = (SELECT id FROM %(temp_schema).road_node_idmap WHERE vendor_id = mov.%(mov_NUMVIANOEUD)::character varying))
						    JOIN tempus.road_section as road_section2 
                                ON (road_section2.node_from = (SELECT id FROM %(temp_schema).road_node_idmap WHERE vendor_id = mov.%(mov_NUMVIANOEUD)::character varying) 
                                   AND road_section2.node_to = (SELECT id FROM %(temp_schema).road_node_idmap WHERE vendor_id = mov.%(mov_NUMNOEUDD)::character varying))
);
