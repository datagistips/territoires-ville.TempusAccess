...
BEGIN
    ...
    THEN 
        RAISE EXCEPTION 'Parameters 1 to 3 must be non-empty arrays';
    END IF;
    ...
    LOOP 
        ...
        ELSE
            indics_time_ag = indics_time_ag || (SELECT coalesce(time_ag_agencies || ' AS ' || r.col_name || ', ', '') FROM tempus_access.indicators WHERE col_name = r.col_name);
            indics_day_ag = indics_day_ag || (SELECT coalesce(replace(day_ag_agencies, '%(day_ag)', day_ag_str::character varying) || ' AS ' || r.col_name || ', ', '') FROM tempus_access.indicators WHERE col_name = r.col_name);         
        END IF;
    END LOOP; 
    ...
    s= $$DROP TABLE IF EXISTS daily_aggregated_data; 
         ...
         (
                WITH all_days AS
                (
                    SELECT DISTINCT s.agency_id_int, s.route_type, q.day_date
                    FROM 
                    (
                    SELECT DISTINCT agency_id_int, route_type
                    FROM filtered_data
                    UNION
                    SELECT DISTINCT agency_id_int, 8
                    FROM filtered_data
                    ) s
                ...
                ( -- Mode by mode
                    SELECT feed_id, 
                           all_days.agency_id_int, 
                           agency_id, 
                           agency_name, 
                           all_days.route_type,
                           all_days.day_date, 
                           $$ || indics_time_ag || $$
                    FROM filtered_data RIGHT JOIN all_days ON (all_days.agency_id_int = filtered_data.agency_id_int AND all_days.route_type = filtered_data.route_type AND all_days.day_date = filtered_data.date) 
        ...
        (
            ...
            (
                SELECT
                   ...
                   route_type,
                   array_remove(array_agg(CASE WHEN agency_id IS NOT NULL THEN day_date END ORDER BY day_date), NULL) as services_days,
                   array_agg(day_date ORDER BY day_date) as days, 
                   $$ || indics_day_ag || $$
                FROM daily_aggregated_data
                GROUP BY agency_id_int, 
                       route_type
                ORDER BY agency_id_int, 
                       route_type
            ) r
        );
    ...
    THEN 
        ...
        s = $$
                ALTER TABLE indic.agencies
                ADD COLUMN zones_list character varying[];
                
                WITH stops_zones AS (
                    SELECT agency.id, array_agg(DISTINCT zoning.vendor_id) as zones_list
                    FROM tempus_gtfs.agency JOIN tempus_gtfs.routes ON (routes.agency_id_int = agency.id)
                                            JOIN tempus_gtfs.trips ON (trips.route_id_int = routes.id)
                                            JOIN tempus_gtfs.stop_times ON (trips.id = stop_times.trip_id_int)
                ...
                UPDATE indic.agencies
                SET zones_list = stops_zones.zones_list
                FROM stops_zones
                WHERE stops_zones.id = agencies.agency_id_int;
            $$;
        EXECUTE(s);
    END IF; 
    ...
    THEN 
        ...
        s = $$
                ...
                WITH stops_zones AS (
                    ...
                    GROUP BY stops_zones.id
                )
                UPDATE indic.agencies
                SET zones_pop = stops_pop.population
                FROM stops_pop
                WHERE stops_pop.id = agencies.agency_id_int;
        $$;
        EXECUTE(s);
    END IF;
