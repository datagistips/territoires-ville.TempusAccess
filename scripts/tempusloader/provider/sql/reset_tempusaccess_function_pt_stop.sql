...
BEGIN
    ...
    s = $$
        DROP TABLE IF EXISTS indic.stops;
        CREATE TABLE indic.stops AS
        (
    ...
    s = $$
            ALTER TABLE indic.stops ADD COLUMN symbol_size real;
            ALTER TABLE indic.stops ADD COLUMN symbol_color real;
            ALTER TABLE indic.stops ADD CONSTRAINT stops_pkey PRIMARY KEY(gid);
        $$;
    RAISE NOTICE '%', s; 
    EXECUTE(s);
    ...
    t=$$
        ...
        VALUES ('stops', 
               (SELECT code FROM tempus_access.obj_type WHERE def_name = 'stops')::character varying, 
